...
export default function NumberSocialLogin({ navigation, route }: any) {
  ...
  const LoginButtonPress = () => {
    ...
    if (!route?.params?.userName?.trim() && userName == "") {
      ...
      if (!response.data.valid) {
        ...
        const data = {
          ...
          userName: route?.params?.userName.trim()
            ...
            : userName,
          email: route?.params?.socialId ? route?.params?.socialId : "",
        };
  ...
  const apiSuccess = (ResponseData: any, ErrorStr: any) => {
    ...
    if (ErrorStr) {
      ...
      let name = route?.params?.userName.trim()
        ? route?.params?.userName
        : userName;
      console.log(
        "name====================================",
  ...
  const SaveData = async () => {
    await AsyncStorage.setItem(
      "filledName",
      route?.params?.userName.trim() ? route?.params?.userName : userName
    );
          ...
          >
            ...
            <View style={styles.formContainer}>
              {!route?.params?.userName.trim() && (
                ...
                >
                  <Text style={styles.nameText}>
                    We could not retrieve your phone number and name from{" "}
                    {route?.params?.signupType
                      ? route.params.signupType.charAt(0).toUpperCase() +
                        route.params.signupType.slice(1)
                      : route?.params?.signupType}
                    .
                  </Text>
                </View>
              ...
              <View style={styles.phTextContainer}>
                {route?.params?.userName.trim() && (
                  <Text style={styles.nameText}>
                    We could not retrieve your phone number from{" "}
                    {route?.params?.signupType
                      ? route.params.signupType.charAt(0).toUpperCase() +
                        route.params.signupType.slice(1)
                      : route?.params?.signupType}
